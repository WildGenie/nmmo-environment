   ...
   def step(self, actions):
      ...
      for entID, ent in self.realm.players.items():
         ...
         if ent.agent.scripted:
            ...
            ###NOTE: COLLECTING REWARDS FOR DEAD MEMBERS OF TEAMS
            for i,team in enumerate(self.teams):
      ...
      for entID, ent in self.dead.items():
         ...
         ###NOTE: COLLECTING REWARDS FOR ALIVE MEMBERS OF TEAMS
         for i,team in enumerate(self.teams):
            if isinstance(team,list):
               x = range(0,len(team))
            elif isinstance(team,int):
      ...
      return obs, rewards, dones, infos #NOTE: rewards gives us [] when getting it in demos.rewards_and_tasks
   ############################################################################
   ### Logging
   def max(self, fn):
